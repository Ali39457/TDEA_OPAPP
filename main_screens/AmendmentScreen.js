...
export default function AmendmentScreen({navigation}) {
...
///////// =========States for store AmendmentDebateArray===========/////////
const [DebateDATA0,SetDebateDATA0]=useState([])
const [AmendmentDebateArray,setAmendmentDebateArray]=useState(DebateDATA0);
const [Debatecount0,setDebatecount0]=useState(1);
const data = [{label: 'Add Details',}];

////////////========method for store AmendmentDebateArray========///////////////

const HandleSubmitDebate=()=>{
  const newArray=[...DebateDATA0,
    {
    AmendmentDebateCount:Debatecount0,
    memberName:SearchAmendmentMember,
    member_id:MemberID,
    AmendmentDescription:AmendmentDescription,
    AmendmentDebateHour:AmendmentDebateHour,
    AmendmentDebateMins:AmendmentDebateMins,
  }]
  setAmendmentDebateArray(newArray)
...
{/*----------------------------------------States----------------------------------------- */}
 const [AmendmentDebateCount,setAmendmentDebateCount]=useState('');
 const [AmendmentDebateMember,setAmendmentDebateMember]=useState("");
 const [AmendmentDescription,setAmendmentDescription]=useState("");
 const [AmendmentDebateHour,setAmendmentDebateHour]=useState("");
 const [AmendmentDebateMins,setAmendmentDebateMins]=useState("");

 const [masterDataDebate,setmasterDataDebate]=useState([]);
 const [SearchAmendmentMember,setSearchAmendmentMember]=useState([]);
...
const getName = async () => {
  ...
  try {
    await AsyncStorage.getItem("@ApiData").then(value=>{if(value!=null){
      ...
      setAmendmentDebateMember((user.member_list))
...
<TouchableOpacity style={styles.RenderItem} onPress={()=>{
  setSearchAmendmentMember(item.name)
  SetMemberID(item.id)
...
if(text){
 ...
 setAmendmentDebateMember(newData);
 setSearchAmendmentMember(text);
}else{ 
 setAmendmentDebateMember(masterDataDebate);
 setSearchAmendmentMember(text);
}
 }
...
const newArray=[...DebateDATA2,{id:countDebate,name:SearchAmendmentMember,party:DebateParty,member_id:MemberID}]
...
{/*---------------------------------------- DebateArray----------------------------------------- */}
var DebateArray=[];
for(let i = 0 ; i < AmendmentDebateCount ; i++ ){
DebateArray.push(
<View key={i}>
...
<TextInput 
placeholder=''
onChangeText={(e) => setAmendmentDescription(e)}
style={styles.TextInput}
/>
...
<Text style={styles.QuestionsText}>Time</Text>
          <TextInput  
           ...
           onChangeText={(value)=>setAmendmentDebateHour(value)} />
          ...
          <TextInput  
           ...
           onChangeText={(value)=>setAmendmentDebateMins(value)} />
</View>
...
//////// =========States for store AmendmentScreenArray===========/////////
  const [DATAA,SetDATAA]=useState([])
  const [AmendmentScreenArray,setAmendmentScreenArray]=useState(DATAA);
  const [countAmendmentScreen,setCountAmendmentScreen]=useState(1);
  
////////////========method for store AmendmentScreenArray========///////////////
  
  const HandleSubmitAmendment=()=>{
    const newArray=[...DATAA,
      {
        amendmentCount:countAmendmentScreen,
        mover_Array:AmendmentMoverArray,
        agendaType:text1.value,
        detail:text3,
        status:text4.value,
        DebateHeld:DebateHeld,
        AmendmentDebateArray:AmendmentDebateArray
    }]
    setAmendmentScreenArray(newArray)
    SetDATAA(newArray)
    setCountAmendmentScreen(countAmendmentScreen+1)
  }
  ...
  const StoreData=async()=>{
    try {
      var user={
        Number_of_Amendment_to_Rules:AmendmentCount,
        Amendment_to_Rules:[...AmendmentScreenArray]
      }
     await AsyncStorage.setItem('Screen-31',JSON.stringify(user))
    } catch (error) {
      console.log(error)
    }
  }
 ...
 {/*---------------------------------------- removeData using AsyncStorage----------------------------------------- */}
  const removeData=async()=>{
    try {
     await AsyncStorage.removeItem('Screen-31')
     navigation.replace('31 of 37')
     console.log("removed")
    } catch (error) {
      console.log(error)
    }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////// AmendmentScreen Code //////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  //Mover Name Array//

  const [MoverDATA0,SetMoverDATA0]=useState([])
  const [AmendmentMoverArray,setAmendmentMoverArray]=useState(MoverDATA0);
  const [Movercount0,setMoverCount0]=useState(1);

  const HandleSubmitMover=()=>{
    const newArray=[...MoverDATA0,
      {
      AmendmentMoverCount:Movercount0,
      moverName:{assembly:AmendmentMoverAssembly,id:AmendmentMoverCount,name:SearchAmendmentMover,party:AmendmentPartyName},
    }]
    setAmendmentMoverArray(newArray)
  ...
  {/*-------------(State-1)-----------------------Amendment-Mover----------------------------------------- */}
  const [text1,setText1]=useState("");
  const [text3,setText3]=useState("");
  const [text4,setText4]=useState("");

  const AmendmentRadioBTn = [{label: 'Check the box if debate was held'}];
  const [DebateHeld,setDebateHeld]=useState('No')

  {/*------------------------------------Modal States----------------------------------------- */}
  const [modalVisible, setModalVisible] = useState(false);

{/*------------------------------------Name of Amendment-Mover states----------------------------------------- */}

const[AmendmentMover,setAmendmentMover]=useState([]);
const [masterData,setMasterData]=useState([]);
const [SearchAmendmentMover,setSearchAmendmentMover]=useState([]);
const [Hide,setHide]=useState(true);
const [Hide2,setHide2]=useState(false);

 {/*------------------------------------useEffect Amendment-Mover----------------------------------------- */}
...
{/*------------------------------------getAmendmentMover method----------------------------------------- */}

const getAmendmentMover = async () => {

  try {
    await AsyncStorage.getItem("@ApiData").then(value=>{if(value!=null){
      var user=JSON.parse(value)
      setAmendmentMover((user.member_list))
...
{/*------------------------------------renderItem for Amendment-Mover ----------------------------------------- */}

const [AmendmentMoverCount,setAmendmentMoverCount]=useState('');
const [AmendmentMoverAssembly,setAmendmentMoverAssembly]=useState('');
const [AmendmentPartyName,setAmendmentPartyName]=useState('')
...
onPress={()=>{
setSearchAmendmentMover(item.name)
setAmendmentMoverCount(item.id)
setAmendmentMoverAssembly(item.assembly)
setAmendmentPartyName(item.party)
setHide(true)
...
{/*------------------------------------ItemSeparator for Amendment-Mover----------------------------------------- */}
...
{/*------------------------------------SearchFilter for Amendment-Mover states----------------------------------------- */}
...
setAmendmentMover(newData);
setSearchAmendmentMover(text);
}else{ 
setAmendmentMover(masterData);
setSearchAmendmentMover(text);
}
}
...
const HandleSubmit=()=>{
  const newArray=[...DATA,{id:count,name:SearchAmendmentMover,party:AmendmentPartyName}]
  SetDATA(newArray)
  setCount(count+1)
...
<View style={{borderWidth:0.5,height:0.5,width:"100%",borderColor:"#CACFD2",marginBottom:5,marginTop:10}}/>
{/*---------------------------------------- Details----------------------------------------- */}
<Text style={styles.QuestionsText}>Details of Amendment</Text>
<TextInput 
 placeholder=''
...
<TouchableOpacity style={styles.AddBTn1} onPress={()=>{ 
                      ...
                      Alert.alert('Please fill detail in all fields')
                        ...
                        setAmendmentDebateArray([])
                        setDebateHeld("No")
                        setMemberName([])
                        setSearchAmendmentMember('')
                        setSearchAmendmentMover('')
                        setMoverName([])
                        SetMoverDATA0([])
                        SetDATA('')
                        setCount(1)
...
<TextInput style={styles.TextInput} keyboardType="numeric" maxLength={2} onChangeText={(value)=> setAmendmentDebateCount(value)} />
...
<View style={{marginBottom:100,marginTop:10}}/>
    ...
    {/*------------------------------------Modal Handle Modal btn----------------------------------------- */}
     ...
     {/*------------------------------------Amendment Modal Code (End)----------------------------------------- */}

const Check=  AmendmentCount.length==0 || text1.length==0 || text3.length==0 || text4.length==0
...
<Text style={styles.Text}>How many amendments to the rules were included on the agenda/moved?</Text>

<View style={styles.TextInputView}>
<TextInput style={styles.TextInput} keyboardType="numeric" maxLength={2}  onChangeText={(value)=>setAmendmentCount(value)} />
</View>

{
  AmendmentCount.length==0 ?  null :<View style={styles.paddingView}>
  <Text style={styles.HeadingText}>Amendments to Rules </Text>
  </View> 
}
...
<Text style={styles.icon1}>➤</Text>
 ...
 <TouchableOpacity onPress={()=>navigation.navigate('30 of 37')} style={styles.backBtn}>
